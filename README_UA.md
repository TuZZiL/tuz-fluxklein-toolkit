...
[Якщо ви новачок, спочатку пройдіть кроки в блоці нижче, а вже потім відкривайте технічні секції.](#почніть-тут)
...
Якщо вам потрібен лише один робочий перший запуск:

1. Встановіть пакет у `ComfyUI/custom_nodes`.
2. Перезапустіть ComfyUI.
3. Додайте **TUZ FLUX LoRA Loader** з категорії `loaders/FLUX`.
4. Залиште `auto_convert` увімкненим, поставте `edit_mode=Auto`, `strength=0.7` і `use_case=Edit`.
5. Запустіть один простий тестовий рендер з однією LoRA.

Якщо нода з'явилася в меню і зображення змінилося, все працює.
...
1. Оберіть файл LoRA
2. Залиште `auto_convert` увімкненим
3. Поставте `edit_mode` → **Auto** (рекомендований старт)
4. Поставте `strength` → **0.7** (безпечне значення за замовчуванням)
5. Залиште `auto_strength` вимкненим для першої спроби
6. Генеруйте!

Все. Auto режим аналізує ваги LoRA і сам обирає найкращий рівень захисту. Якщо ви редагуєте референсне зображення, залишайте `use_case=Edit`. Далі підлаштовуйте під себе.

---

![Скріншот нод TUZ FLUX](images/nodes.png)

*На скріншоті видно loader і графічний віджет у ComfyUI.*

## Короткий словник

Це терміни, які новачки бачать найчастіше. Якщо запам'ятаєте лише одне правило, залишайте `auto_convert` увімкненим і стартуйте з `edit_mode=Auto`.
...
### Діал захисту (`protection`, legacy `balance`)
...
**Правило:**
- LoRA перезаписує обличчя? → Збільшуйте `protection` до 1.0
- Edit занадто слабкий або "перестрахований"? → Зменшуйте `protection` до 0.0
...
| `protection` | float | "Сира" LoRA ↔ повний захист пресету (`balance` все ще приймається для сумісності) |
...
| `recommended_balance` | FLOAT | Рекомендований рівень захисту (legacy output key) |
...
Примітка: цей `balance` у ноді окремий від loader `protection` і його legacy `balance`-аліаса.
...
## Рецепти для новачків

### Рецепт 1: Перша LoRA, безпечний старт

```
LoRA Loader: auto_convert=ON, use_case=Edit, edit_mode=Auto, strength=0.7, protection=0.5
```

Це варіант для першої перевірки, чи нода працює. Якщо ефект занадто сильний, спочатку зменшіть `strength`.

### Рецепт 2: Редагування одягу зі збереженням обличчя

```
LoRA Loader: auto_convert=ON, use_case=Edit, edit_mode=Preserve Face, strength=0.65-0.75, protection=0.7
```

Використовуйте це, коли LoRA змінює одяг або аксесуари, але людина має лишатися впізнаваною.

### Рецепт 3: Простий multi-LoRA стек
...
Якщо потрібна стабільність тіла, залишайте `anatomy_profile` лише на Slot 1. Ставтеся до `protection` як до головного регулятора, а `balance` згадуйте лише тоді, коли відкриваєте старий workflow.

---

## Усунення проблем

- LoRA майже не впливає на результат: перевірте, що `auto_convert` увімкнено, модель завантажена, а файл LoRA лежить у `models/loras`. Якщо файл з diffusers, конвертер краще не вимикати.
- Auto вибрав не той пресет: переключіться з `Auto` на `Preserve Face` або `None`. Auto робить найкращу здогадку, але не читає ваш намір.
- Невідповідність model/LoRA: цей пакет зроблений для FLUX.2 Klein і також працює з FLUX.1. Якщо ви використовуєте GGUF, встановіть [ComfyUI-GGUF](https://github.com/city96/ComfyUI-GGUF).
- Відсутня `TUZ Klein Edit Composite`: встановіть `opencv-python-headless`.
- Зміни є, але занадто слабкі: спочатку підніміть `strength`, а вже потім міняйте пресет.

---

## Додатковий технічний довідник (опційно)
...
A: Auto не читає ваш намір. Переключіться на ручний: `Preserve Face` для identity-роботи, `None` для повної свободи. Підлаштовуйте `protection` як діал захисту (`balance` лишився legacy-аліасом).
